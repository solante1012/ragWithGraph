...
public abstract class AbstractLLMService<T> extends CommonModelService<T> {
    ...
    public ChatModel buildChatLLM(SseAskParams params, String uuid) {
        LLMBuilderProperties tmpProperties = params.getLlmBuilderProperties();
        if (null == tmpProperties) {
            tmpProperties = new LLMBuilderProperties();
            tmpProperties.setTemperature(0.7);
            tmpProperties.setReturnThinking(false);
            params.setLlmBuilderProperties(tmpProperties);
    ...
    /**
     * 普通聊天，将原始的用户问题及历史消息发送给AI ， 大模型对话入口 （核心）
     *
     * @param params   请求参数
    ...
    public ChatResponse chat(SseAskParams params) {
        ...
        ChatModelParams chatModelParams = params.getChatModelParams();
        ChatModel chatModel = buildChatLLM(params, params.getUuid());
        List<ChatMessage> chatMessages = createChatMessages(chatModelParams);
        ChatRequest chatRequest = createChatRequest(chatModelParams.getMcpClients(), chatMessages, params.getLlmBuilderProperties().getReturnThinking());
